...
try:
    from backend.knowledge_base import add_document
except Exception:
...
async def proxy_upload_rag(request: Request, file: UploadFile = File(...), user=Depends(require_login("admin"))):
    ...
    and indexes it into the knowledge base by calling backend.knowledge_base.add_document.
    ...
    if add_document is None:
        ...
        try:
            from backend.knowledge_base import add_document as _ad
            globals()['add_document'] = _ad
        except Exception:
            raise HTTPException(status_code=500, detail="Knowledge base not available on this server")
    ...
    metadata = {"source": "upload_proxy", "filename": filename}
...
def download_knowledge_file(request: Request, filename: str, user=Depends(require_role("admin", "employee"))):
