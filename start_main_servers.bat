@echo off
setlocal
REM Ensure Python uses UTF-8 for stdout/stderr to avoid UnicodeEncodeError
set PYTHONUTF8=1
...
REM ASSISTIFY_COLLECTION_NAME is intentionally NOT set — let auto-discovery
REM find the latest non-empty collection (Blue/Green indexing creates new ones).
...
echo   (RAG + Login only)
echo ====================================
echo Safe Mode Flags: SAFE_MODE=1 TTS=off RERANKER=off WHISPER=off WARMUP=on COLLECTION=auto DOC_MODE=single DOMAIN_HEURISTICS=off EMBED_DEVICE=cuda
