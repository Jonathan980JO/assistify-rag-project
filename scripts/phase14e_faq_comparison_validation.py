"""Phase 14E - FAQ extraction, comparison entity parsing, and table preservation validation."""
...
from backend.retrieval.routing import _compare_entities_from_query, _extract_best_faq_answer
...
TABLE_QUERIES: list[dict[str, str]] = [
    {"id": "tbl_vm_memory", "group": "table", "query": "Which VM family has the most memory?"},
    {"id": "tbl_vm_ml", "group": "table", "query": "Which VM family is best for machine learning?"},
    {"id": "tbl_hipaa", "group": "table", "query": "Which plan includes HIPAA support?"},
    {"id": "tbl_sso_plans", "group": "table", "query": "Which plans support SSO?"},
]

VALIDATION_QUESTIONS: list[dict[str, Any]] = FAQ_QUERIES + COMPARE_QUERIES + TABLE_QUERIES
...
def run_offline_checks() -> tuple[int, list[dict[str, Any]]]:
    ...
    for question in COMPARE_QUERIES:
        ...
        expected = question["expected_entities"]
...
async def run_validation(args: argparse.Namespace) -> int:
    ...
    json_path = output_dir / "phase14e_faq_comparison_validation.json"
    md_path = output_dir / "phase14e_faq_comparison_validation.md"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Phase 14E FAQ/comparison/table validation.")
...
def main() -> int:
    ...
    if args.offline_only:
        ...
        for item in offline_results:
            ...
            print(f"{status} {item['id']} entities={item.get('entities')}")
