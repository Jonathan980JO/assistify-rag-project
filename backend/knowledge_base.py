...
def _embedding_text_with_structure(text: str, metadata: dict | None) -> str:
    """Prefix embedding text with generic document-structure metadata."""
    parts: list[str] = []
    md = metadata or {}
    field_labels = (("chapter", "Chapter"), ("section", "Section"), ("title", "Title"))
    seen: set[str] = set()
    for key, label in field_labels:
        value = " ".join(str(md.get(key) or "").split())
        if not value or value.lower() in {"document", "page"}:
            continue
        low = value.lower()
        if low in seen:
            continue
...
def chunk_and_add_document(
    ...
    for batch_start in range(0, len(chunks), UPSERT_BATCH_SIZE):
        ...
        try:
            ...
            embedding_inputs = [
                _e5_passage(
                    _embedding_text_with_structure(
                        chunk,
                        {
                            "chapter": record.get("chapter"),
                            "section": record.get("section"),
                            "title": record.get("title") or record.get("heading"),
                        },
                    )
                )
                for chunk, record in zip(batch_chunks, batch_records)
            ]
